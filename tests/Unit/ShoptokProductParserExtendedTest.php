...
class ShoptokProductParserExtendedTest extends TestCase
{
    ...
    public function test_handles_products_without_images()
    {
        ...
        $items = $crawler->filter('.product-item');
    ...
    public function test_generates_valid_external_ids()
    {
        ...
        $crawler1 = new \Symfony\Component\DomCrawler\Crawler($html1);
        $crawler2 = new \Symfony\Component\DomCrawler\Crawler($html2);

        $result1 = $this->parser->parseItem($crawler1->filter('.product-item')->first(), 'https://example.com');
        $result2 = $this->parser->parseItem($crawler2->filter('.product-item')->first(), 'https://example.com');

        $this->assertNotNull($result1['external_id']);
        $this->assertNotNull($result2['external_id']);
        // Different product URLs should generate different IDs
        $this->assertNotEquals($result1['external_id'], $result2['external_id']);
    }
    ...
    public function test_normalizes_urls_correctly()
    {
        ...
        $result = $this->parser->parseItem($crawler->filter('.product-item')->first(), 'https://www.shoptok.si');

        $this->assertStringStartsWith('https://www.shoptok.si', $result['product_url']);
    }
